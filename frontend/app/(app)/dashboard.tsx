...
export default function DashboardScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    const futureMeetups = allMeetups
      .filter((m) => m.status === 'active' && m.scheduled_time > now)
      .sort((a, b) => a.scheduled_time.localeCompare(b.scheduled_time));

    const upcomingMeetup = futureMeetups[0] ?? null;
    const nextNext = futureMeetups[1] ?? null;

    setMeetup(upcomingMeetup);
    setNextNextMeetup(nextNext);
  ...
  const handleRSVP = async (intent: 'going' | 'not_going') => {
    if (!user || !meetup) return;
    setRsvpLoading(true);
    try {
      const updated = await api.upsertRSVP(meetup.id, user.id, intent);
      setRsvp(updated);
    } catch {
          ...
          <View style={styles.actionArea}>
            {isCanceled ? (
              ...
              <>
                ...
                {rsvpLoading && <ActivityIndicator color={colors.primary} style={{ marginTop: spacing.md }} />}
              </>
            )}
          </View>
        )}

        {/* ── Next-Next Meetup Preview ──────────────────────────────────── */}
        {nextNextMeetup && (
